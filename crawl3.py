...
# Output file to save crawled links
output_file = "crawled_links.txt"

# Set to keep track of visited links
visited_links = set()

def crawl_website_iteratively(start_url, release):
    """
    Iteratively crawls the given URL, processes only links within the target domain,
    and containing the release pattern, saving all unique links to a file.
    """
    ...
    with open(output_file, 'w') as file:  # Open file once for writing
        while to_visit:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if release in url:  # Save only if the URL contains the release pattern
                        file.write(url + '\n')
...
crawl_website_iteratively(base_url, release_pattern)

print(f"\nCrawling completed! All unique links from '{urlparse(base_url).netloc}' with release '{release_pattern}' saved in '{output_file}'.")
